...
import {
  ...
  SPOTIFY_CURRENT_PLAYBACK_URL,
  SPOTIFY_TOKEN_URL,
} from "./model";
export const getSpotifyRouteMessage = (): string => "Spotify route";

const spotifyLogger = Logger.fmtPackage("SPOTIFY");
...
async function refreshSpotifyToken(): Promise<string> {
  ...
  if (!response.ok) {
    ...
    Logger.error(
      `${spotifyLogger}Failed to refresh Spotify token: ${JSON.stringify(errorData)}`,
    );
  ...
  if (!json.access_token || !json.expires_in) {
    Logger.error(
      `${spotifyLogger}Invalid response from Spotify token refresh: ${JSON.stringify(json)}`,
    );
  ...
  Logger.success(`${spotifyLogger}Successfully refreshed Spotify token`);
...
export async function getCurrentSpotifyPlayback(
  ...
  if (response.status === 401) {
    ...
    Logger.warning(
      `${spotifyLogger}Spotify token might have expired, refreshing and retrying...`,
    );
    await refreshSpotifyToken();
    if (retry) {
      Logger.error(
        `${spotifyLogger}Failed to get current Spotify playback after retrying.`,
      );
      return null;
    }
  ...
  if (response.status === 403) {
    ...
    Logger.error(
      `${spotifyLogger}Couldn't get playback info (Forbidden). Might have to reset your OAuth.`,
    );
    return null;
  }
  ...
  if (response.status === 429) {
    ...
    Logger.error(
      `${spotifyLogger}Rate limited by Spotify when trying to get current playback.`,
    );
    return null;
  }
  ...
  if (!response.ok) {
    Logger.error(
      `${spotifyLogger}Failed to get current Spotify playback: ${response.statusText}`,
    );
    return null;
  }
